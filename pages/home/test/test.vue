...
<script setup>
	...
	function onTouchMove(e, index) {
		...
		if (e.touches.length === 1 && !isRotatingOrScaling) {
			...
			updateEditRegionToolCoordinate({ x: item.x, y: item.y }, item.type);
			// 判断是否触碰到边界
			// if (item.x < 0) {
			// 	item.x = 0;
			// }
			...
			// 	item.y = 0;
			// }
			// if (item.x + item.width > 300) {
			// 	item.x = 300 - item.width;
			// }
			// if (item.y + item.height > 300) {
			// 	item.y = 300 - item.height;
			// }
		}
	}
	...
	function scaleSourceMaterial(e, index) {
		if (e.touches.length === 1) {
			...
			updateEditRegionToolCoordinate({ scale: item.scale }, item.type);
		}
	}
